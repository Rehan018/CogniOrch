...
add_alias_to_bashrc() {
    if ! grep -Fxq "# CogniOrch Integration" ~/.bashrc; then
        echo "" >>~/.bashrc
        echo "# CogniOrch Integration" >>~/.bashrc
...
echo "Installation complete. Please restart your terminal or run 'source ~/.bashrc' to use CogniOrch. Enjoy :)"