...
# Get the parent directory to import CogniOrch modules
...
class TerminalProtocolHandler(ProtocolHandler):
    ...
    def handle(self, command: str, require_approval: bool, auto_approve: bool) -> Dict[str, Any]:
        ...
        try:
            ...
            is_safe, reason = CommandVerifier.verify(command)
            
            if not is_safe:
                logger.warning(f"Command safety check failed: {reason}")
                print(f"\n\033[1;31m[SAFETY WARNING] {reason}\033[0m")
                # We still allow the user to approve, but we warn them clearly
