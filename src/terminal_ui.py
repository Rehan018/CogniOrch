"""
An improved terminal user interface for CogniOrch using prompt_toolkit.
...
COGNIORCH_STYLE = Style.from_dict({
...
class ImprovedTerminalUI:
    """A more sophisticated terminal UI for CogniOrch."""

    def __init__(self, ai, config):
        """Initialize the terminal UI with CogniOrch instance and config."""
        self.cogniorch_ai = ai
        self.config = config
        self.commands = ['help', 'history', 'clear', 'exit']

        # Create history file in user's home directory
        history_file = os.path.expanduser('~/.cogniorch_history.txt')
    ...
    def print_banner(self):
        """Print CogniOrch welcome banner."""
        banner = """
...
<info>Available commands:</info>
  ...
  • <highlight>exit</highlight>    - Exit CogniOrch
...
<info>Tips:</info>
  ...
  • Use <highlight>Ctrl+D</highlight> to exit
        """
        print_formatted_text(HTML(help_text), style=COGNIORCH_STYLE)
    ...
    def format_ai_response(self, response):
        ...
        # Handle MCP terminal tags
        if '<mcp:terminal>' in formatted:
            formatted = re.sub(
    ...
    def display_history(self):
        ...
        print_formatted_text(HTML('<b><u>Conversation History:</u></b>'), style=COGNIORCH_STYLE)

        history = self.cogniorch_ai.get_conversation_history()
        if not history:
            print_formatted_text(HTML('<i>No conversation history yet.</i>'), style=COGNIORCH_STYLE)
            return

        for i, entry in enumerate(history, 1):
            role = "You" if entry["role"] == "user" else "CogniOrch"
            content = entry["content"]

            # Format content based on role
            if role == "CogniOrch":
                content = self.format_ai_response(content)
                print_formatted_text(HTML(f'<b><ansiblue>{role}:</ansiblue></b> {content}'), style=COGNIORCH_STYLE)
            else:
                print_formatted_text(HTML(f'<b><ansigreen>{role}:</ansigreen></b> {content}'), style=COGNIORCH_STYLE)

            # Add separator between messages
            if i < len(history):
                print_formatted_text(HTML('<ansigray>─────────────────────────────────────</ansigray>'), style=COGNIORCH_STYLE)
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                prompt_text = HTML(f'<prompt>{username}@cogniorch</prompt> > ')

                # Get user input with the session
                user_input = self.session.prompt(prompt_text, style=COGNIORCH_STYLE)
                ...
                if user_input.lower() == 'exit':
                    print_formatted_text(HTML('<ansired>Goodbye! Have a great day!</ansired>'), style=COGNIORCH_STYLE)
                    break
                ...
                print_formatted_text(HTML('\n<ansired>Goodbye! Have a great day!</ansired>'), style=COGNIORCH_STYLE)
                break

            except Exception as e:
                print_formatted_text(HTML(f'<error>Error: {str(e)}</error>'), style=COGNIORCH_STYLE)