"""
Persistent terminal executor for CogniOrch.
...
class PersistentTerminalExecutor:
    ...
    def __init__(self):
        ...
        self.output_file = os.path.join(self.temp_dir, "cogniorch_command_output.txt")
        self.lock_file = os.path.join(self.temp_dir, "cogniorch_command_lock")
        self.pid_file = os.path.join(self.temp_dir, "cogniorch_terminal_pid.txt")
        self.fifo_path = os.path.join(self.temp_dir, "cogniorch_terminal_fifo")
        ...
        logging.basicConfig(
            ...
            filename=os.path.join(self.temp_dir, "cogniorch_command.log"),
            filemode='a'
        )
        ...
        if self._is_terminal_running():
            ...
            logging.info("Found an existing CogniOrch terminal session, will reuse it.")
        ...
        try:
            ...
            try:
                with open(f"/proc/{pid}/cmdline", 'r') as cmd_file:
                    ...
                    if "cogniorch_terminal_script" not in cmdline:
                        return False
    ...
    def _initialize_terminal(self):
        ...
        try:
            ...
            with open(script_path, 'w') as f:
                f.write('''#!/bin/bash
echo $$ > %s
echo "CogniOrch Terminal - DO NOT CLOSE THIS WINDOW"
echo "This terminal will be used for all CogniOrch commands."
echo "---------------------------------------------------"
...
''' % (self.pid_file, self.fifo_path, self.lock_file,
    ...
    def _initialize_fallback(self):
        ...
        script_path = os.path.join(self.temp_dir, "cogniorch_fallback_script.sh")
        with open(script_path, 'w') as f:
            f.write(f'''#!/bin/bash
echo $$ > {self.pid_file}
echo "CogniOrch Fallback Terminal"
while true; do
    sleep 1
done
''')
